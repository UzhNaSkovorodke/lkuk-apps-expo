...
const ProfileScreen = ({ navigation, profile, logoutProfile, editProfile, setError }) => {
    ...
    const selectProfileImage = async () => {
        ...
        if (status !== 'granted') {
            setError([
                {
                    message: 'Доступ к фото запрещен. Дать доступ можно в настройках приложения.',
                },
            ])
            return
        }
    ...
    return (
        <ScrollView scrollEventThrottle={16}>
            <View style={styles.container}>
                ...
                {renderContacts()}
                <DefaultButton onPress={onPasswordChangeButtonPress} text="Изменить пароль" />
                <TouchableOpacity
                ...
                <TouchableOpacity style={styles.profileExitButton} onPress={onLogOutButtonPress}>
