...
const SYSTEM_ERROR_MESSAGE = 'Извините, во время работы системы произошла ошибка. Попробуйте позже.';
const AUTH_ERROR_MESSAGE = 'Не удалось авторизоваться в системе. Проверьте введенные данные.';
const RESET_ERROR_MESSAGE = 'Не удалось отправить письмо для восстановления пароля. Пожалуйста, проверьте введенные данные.';
const DISCONNECT_ERROR_MESSAGE = 'Нет соединения. Проверьте подключение к интернету';

const objToString = (data = {}) => Object.keys(data)
...
const removeMimeTypeFromBase64 = (str) => {
  if (str.slice(0, 5) === 'data:') return str.split(',')[1];
  ...
  return (
    fetch(
      ...
      .catch(({ message }) => Promise.reject([
        { message: DISCONNECT_ERROR_MESSAGE }
      ]))
      ...
      .then(({ response, json }) => {
        ...
        if (json && json.message) return Promise.reject([{ message: json.message }]);
        ...
        if (response.status >= 500) message = SYSTEM_ERROR_MESSAGE;
...
const fetchDict = (name, additionalParams) => () => send({
  name,
  queryParams: 'fake=di',
  body: `{
    getDictionaries {
...
export const logout = token => send({
  method: 'GET',
  token,
...
export const resetPassword = (token, data) => send({
  token,
  queryParams: 'r=reset-new-password',
  body: `mutation {
    resetNewPassword(${data.login ? `login: "${data.login}"` : `email: "${data.email}"`
    })
  }`
}).then(res => (!res || !res.data.resetNewPassword
  ? Promise.reject([
    {
...
export const changePassword = (token, { password, newPassword }) => send({
  token,
  body: `mutation {
...
export const editProfile = (token, { fileName, fileContent }) => send({
  token,
  body: `mutation {
  editProfile(
    avatar : {
...
export const editProfilePushNotifcations = (token, { enablePush, fcmToken }) => send({
  token,
  body: `mutation {
...
export const fetchProfile = token => send({
  token,
  body: `{
    profile {
      fio,
...
export const fetchProjects = token => send({
  token,
  // urlParams: '/projects',
  body: `{
    getRealEstate {
...
export const fetchDocs = (token, { projectId }) => send({
  token,
...
export const fetchRoom = (token, { roomId }) => send({
  token,
...
// ex fetchRooms
export const fetchRoomsForProject = (token, { projectId }) => send({
  token,
  // urlParams: '/rooms/index.php',
  body: `{
...
export const fetchAllRooms = (token, { projectId }) => send({
  token,
  // urlParams: '/rooms/index.php',
  body: `{
...
export const fetchNews = (token, { newsId }) => send({
  token,
...
export const fetchAllNews = (token, { page = 0, size = 10 } = {}) => send({
  token,
  queryParams: 'fake=news',
  body: `{
...
export const updateNotification = (token, { notificationId } = {}) => send({
  token,
...
export const fetchServices = (token, { projectId }) => send({
  token,
...
// TODO: проверить
export const sendComment = (token, { eventId, text, files }) => send({
  token,
  queryParams: 'fake=news',
  body: `mutation {
    postComment(
...
// TODO: проверить
export const fetchBill = (token, { billId }) => send({
  token,
...
// TODO: проверить
export const fetchGuestPass = (token, { eventId }) => send({
  token,
  body: `{
...
// TODO: проверить
export const fetchDeliveryPass = (token, { eventId }) => send({
  token,
  body: `{
    getEvent(eventId: ${eventId}) {
      eventId,
...
export const fetchTaxiPass = (token, { eventId }) => send({
  token,
  body: `{
    getEvent(eventId: ${eventId}) {
      eventId,
...
// TODO: проверить
export const fetchServiceCompanyAppeal = (token, { eventId }) => send({
  token,
  body: `{
    getEvent(eventId: ${eventId}) {
      eventId,
...
export const fetchEditProfileAppeal = (token, { eventId }) => send({
  token,
  body: `{
    getEvent(eventId: ${eventId}) {
      eventId,
...
export const fetchFundsFlow = (token, {
...
export const fetchBillPaymentLink = (token, { billId }) => send({
  token,
  body: `{
...
export const fetchRoomPaymentLink = (token, { roomId, total }) => send({
  token,
  body: `{
    getRoomPayment(
...
export const fetchEventStatuses = () => send({
  queryParams: 'fake=di',
  body: `{
    getDictionaries {
      eventStatuses
...
export const fetchFundsFlowTypes = () => send({
  queryParams: 'fake=di',
  body: `{
    getDictionaries {
      fundsFlowTypes
...
export const editAppealRelevance = (token, { eventId, status }) => send({
  token,
  queryParams: 'fake=event',
  body: `
  mutation {
